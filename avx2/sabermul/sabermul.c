...
void load_values() {
    ...
    int16_t inv3_avx_load[16], inv9_avx_load[16], inv15_avx_load[16],
        int45_avx_load[16], int30_avx_load[16], int0_avx_load[16];
...
void saber_matrix_vector_mul(polyvec *r, const polyvec a[KEM_K],
                             const polyvec *v) {
    int64_t i, j, k;
    ...
    for (i = 0; i < KEM_K; i++)
        for (j = 0; j < KEM_N / 16; j++)
            sk_avx[i][j] =
                _mm256_loadu_si256((__m256i const *)&v->vec[i].coeffs[16 * j]);

    for (i = 0; i < KEM_K; i++)
        for (j = 0; j < KEM_K; j++)
            for (k = 0; k < KEM_N / 16; k++)
                a1_avx_combined[i][j][k] = _mm256_loadu_si256(
                    (__m256i const *)&a[i].vec[j].coeffs[k * 16]);
    ...
    for (i = 0; i < NUM_POLY; i++) {
        for (j = 0; j < NUM_POLY; j++) {
            toom_cook_4way_avx_n1(a1_avx_combined[i][j], b_bucket[j], c_bucket,
                                  j);
        }
    ...
    for (i = 0; i < KEM_K; i++)
        for (j = 0; j < KEM_N / 16; j++)
            _mm256_maskstore_epi32((int *)&r->vec[i].coeffs[16 * j], mask_load,
                                   res_avx[i][j]);
}
...
void saber_iprod(poly *r, const polyvec *a, const polyvec *v) {
    ...
    for (i = 0; i < KEM_K; i++)
        for (k = 0; k < KEM_N / 16; k++)
            a1_avx_combined[0][i][k] =
                _mm256_loadu_si256((__m256i const *)&a->vec[i].coeffs[k * 16]);
    ...
    for (j = 0; j < KEM_N / 16; j++)
        _mm256_maskstore_epi32((int *)&r->coeffs[16 * j], mask_load,
                               res_avx[0][j]);
}
