...
void KARA_interpol(__m256i *c_bucket, __m256i *result_final0,
                   __m256i *result_final1, __m256i *result_final2,
                   __m256i *result_final3, __m256i *result_final4,
                   __m256i *result_final5, __m256i *result_final6);
void KARA_eval(__m256i *b, __m256i *b_bucket);
void TC_eval(__m256i *b_avx, __m256i *b_bucket);

// all 7 Karatsuba evaluation and interpolation are done in AVX.
void batch_64coefficient_multiplications_new(__m256i *a, __m256i *b_bucket,
                                             __m256i *c_bucket, int f) {
    //SCM_SIZE = 16; Holds evaluation (a & b) for 7 Karatsuba at a time
    __m256i a_bucket[SCM_SIZE * 4];
    ...
    if (f == 0) {
        ...
        schoolbook_avx_new2(a_bucket + 16, b_bucket + 16,
                            c_bucket + 2 * SCM_SIZE);
        schoolbook_avx_new2(a_bucket + 32, b_bucket + 32,
                            c_bucket + 4 * SCM_SIZE);
        schoolbook_avx_new2(a_bucket + 48, b_bucket + 48,
                            c_bucket + 6 * SCM_SIZE);
    } else {
        schoolbook_avx_new3_acc(a_bucket, b_bucket, c_bucket);
        schoolbook_avx_new3_acc(a_bucket + 16, b_bucket + 16,
                                c_bucket + 2 * SCM_SIZE);
        schoolbook_avx_new3_acc(a_bucket + 32, b_bucket + 32,
                                c_bucket + 4 * SCM_SIZE);
        schoolbook_avx_new3_acc(a_bucket + 48, b_bucket + 48,
                                c_bucket + 6 * SCM_SIZE);
    }
}
...
void KARA_interpol(__m256i *c_bucket, __m256i *result_final0,
    ...
    //int64_t i;
    register __m256i res_avx0, res_avx1, res_avx2, res_avx3, res_avx4, res_avx5,
        res_avx6, res_avx7; // to hold each 64X64 poly mul results

    __m256i temp, c6_avx, c7_avx, c8_avx, c20_avx, c21_avx, c22_avx, c23_avx,
        c24_avx;
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[21], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[24], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[20], res_avx1), res_avx3);
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[30], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[49], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[29], res_avx1), res_avx3);
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[55], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[58], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[54], res_avx1), res_avx3);
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[80], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[83], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[63], res_avx1), res_avx3);
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[89], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[92], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[88], res_avx1), res_avx3);
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[114], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[117], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[113], res_avx1), res_avx3);
    ...
    c21_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[123], res_avx5), res_avx7);

    c24_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[126], c22_avx), c23_avx);

    c20_avx =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[122], res_avx1), res_avx3);

    temp =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[107], res_avx4), res_avx6);
    res_avx5 = _mm256_add_epi16(res_avx5, temp);

    temp =
        _mm256_sub_epi16(_mm256_sub_epi16(c_bucket[106], res_avx0), res_avx2);
    res_avx1 = _mm256_add_epi16(res_avx1, temp);
...
void toom_cook_4way_avx_n1(__m256i *a_avx, __m256i *b_bucket, __m256i *c_bucket,
                           int f) {
    ...
    batch_64coefficient_multiplications_new(aw_avx, b_bucket, c_bucket,
                                            f); //New
}
...
void TC_interpol(__m256i *c_bucket, __m256i *res_avx) {
    ...
    register __m256i r0_avx, r1_avx, r2_avx, r3_avx, r4_avx, r5_avx, r6_avx,
        temp_avx;

    __m256i w1_avx[2 * small_len_avx], w2_avx[2 * small_len_avx],
        w3_avx[2 * small_len_avx], w4_avx[2 * small_len_avx],
        w5_avx[2 * small_len_avx], w6_avx[2 * small_len_avx],
        w7_avx[2 * small_len_avx];
    ...
    KARA_interpol(c_bucket, w1_avx, w2_avx, w3_avx, w4_avx, w5_avx, w6_avx,
                  w7_avx);
    ...
    for (i = 0; i < 2 * small_len_avx; i++) {
        ...
        if (i < small_len_avx) {
            ...
            res_avx_output[0 * small_len_avx + i] =
                _mm256_add_epi16(res_avx_output[0 * small_len_avx + i], r6_avx);
            res_avx_output[1 * small_len_avx + i] =
                _mm256_add_epi16(res_avx_output[1 * small_len_avx + i], r5_avx);
            res_avx_output[2 * small_len_avx + i] =
                _mm256_add_epi16(res_avx_output[2 * small_len_avx + i], r4_avx);
            res_avx_output[3 * small_len_avx + i] =
                _mm256_add_epi16(res_avx_output[3 * small_len_avx + i], r3_avx);
            res_avx_output[4 * small_len_avx + i] =
                _mm256_add_epi16(res_avx_output[4 * small_len_avx + i], r2_avx);
            res_avx_output[5 * small_len_avx + i] =
                _mm256_add_epi16(res_avx_output[5 * small_len_avx + i], r1_avx);
            res_avx_output[6 * small_len_avx + i] = r0_avx;
        }
    }
    ...
    for (i = 0; i < 16; i++)
        res_avx[i] =
            _mm256_sub_epi16(res_avx_output[i], res_avx_output[i + 16]);
}
